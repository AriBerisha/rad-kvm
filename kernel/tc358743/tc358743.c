...
/*
 * The Qualcomm CCI camera-I2C controller has a small max transfer length and
 * rejects long transfers with -EOPNOTSUPP, which breaks the 128-byte EDID block
 * reads/writes (the source then sees no EDID). Split every transfer into
 * CCI-safe bursts; the chip auto-increments the register address, so chunking
 * is transparent. (q6a-kvm patch.)
 */
#define I2C_BURST 8
...
static int i2c_rd(struct v4l2_subdev *sd, u16 reg, u8 *values, u32 n)
{
	...
	for (off = 0; off < n; off += I2C_BURST) {
		u32 chunk = min_t(u32, n - off, I2C_BURST);
		u8 buf[2] = { (reg + off) >> 8, (reg + off) & 0xff };
		struct i2c_msg msgs[] = {
			{
				.addr = client->addr,
				.flags = 0,
		...
		int err = i2c_transfer(client->adapter, msgs, ARRAY_SIZE(msgs));

		if (err != ARRAY_SIZE(msgs)) {
			v4l2_err(sd, "%s: reading register 0x%x from 0x%x failed: %d\n",
					__func__, reg + off, client->addr, err);
			return 1;
		}
	}
	return 0;
...
static int tc358743_probe_of(struct tc358743_state *state)
{
	...
	/*
	 * Expose the CSI-2 line rate to downstream receivers (e.g. qcom-camss)
	 * via V4L2_CID_LINK_FREQ. The bridge runs its CSI TX at this fixed rate
	 * regardless of input resolution, so report the DT value directly.
	 */
	state->link_freq_menu[0] = endpoint.link_frequencies[0];
...
static int tc358743_probe(struct i2c_client *client)
{
	...
	state->link_freq_ctrl = v4l2_ctrl_new_int_menu(&state->hdl, NULL,
			V4L2_CID_LINK_FREQ, 0, 0, state->link_freq_menu);
	if (state->link_freq_ctrl)
		state->link_freq_ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY;
	...
	/*
	 * qcom-camss only treats MEDIA_ENT_F_CAM_SENSOR entities as the
	 * link-frequency source (see camss_find_sensor_pad): it walks upstream
	 * from the CSIPHY and stops only at a CAM_SENSOR. Declaring the bridge
	 * as MEDIA_ENT_F_VID_IF_BRIDGE makes that walk return NULL, so camss
	 * never reads our V4L2_CID_LINK_FREQ and stream-on fails with -EINVAL.
	 * Masquerade as a sensor so the receiver can find the link frequency.
	 */
