...
func main() {
	...
	mouseRelDev := flag.String("mouse-rel", "/dev/hidg2", "HID relative-mouse gadget device (\"\" to disable)")
	...
	hid, err := NewHID(*kbdDev, *mouseDev, *mouseRelDev)
	if err != nil {
		log.Fatalf("open HID gadget devices: %v\n(is the gadget up — /dev/hidg0,1 — and are you root?)", err)
	}
	if *mouseRelDev != "" && !hid.HasRelMouse() {
		log.Printf("WARN: relative mouse %s unavailable — relative mode disabled (is the gadget current?)", *mouseRelDev)
	}
	defer hid.Close()
	...
	mux.HandleFunc("/api/status", func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, map[string]bool{
			...
			"authed":     auth.valid(r),
			"totp":       auth.TOTPEnabled(), // so the login form knows to ask for a code
			"relmouse":   hid.HasRelMouse(),  // so the UI hides the Relative toggle if absent
		})
	})
	// /api/2fa/new : mint a fresh per-device TOTP secret + provisioning URI for the
	// setup screen to render as a QR / manual key. Stateless — nothing is saved
	// until /api/setup confirms a valid code. Only reachable pre-account or when
	// already signed in, never to an unauthenticated stranger on a live appliance.
	mux.HandleFunc("/api/2fa/new", func(w http.ResponseWriter, r *http.Request) {
		if auth.Configured() && !auth.valid(r) {
		...
		writeJSON(w, map[string]string{
			...
			"uri":     otpauthURI(r.URL.Query().Get("user"), secret),
		})
	})
	mux.HandleFunc("/api/setup", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := auth.Setup(c.Username, c.Password, c.TOTPSecret, c.Code); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		auth.issue(w)
		w.WriteHeader(http.StatusNoContent)
		log.Printf("account created: %q (2FA %s)", c.Username, map[bool]string{true: "on", false: "off"}[c.TOTPSecret != ""])
	})
	mux.HandleFunc("/api/login", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := auth.Login(c.Username, c.Password, c.Code); err != nil {
			time.Sleep(500 * time.Millisecond) // throttle brute force (password + TOTP)
	...
	log.Printf("  video %s ; HID -> %s + %s + %s ; auth %s", *stream, *kbdDev, *mouseDev, *mouseRelDev, *authFile)
	if !auth.Configured() {
...
type creds struct{ Username, Password, Code, TOTPSecret string }

// readCreds decodes the auth JSON body, enforcing POST and a size limit.
// Code and TOTPSecret are optional (used only for two-factor login/setup).
func readCreds(w http.ResponseWriter, r *http.Request) (creds, bool) {
	...
	return creds{c.Username, c.Password, c.Code, c.TOTPSecret}, true
}
