...
apt-get update -y
# --no-install-recommends keeps the on-device footprint small (matters most inside
# the slim image build, where the rootfs partition is tight).
apt-get install -y --no-install-recommends build-essential device-tree-compiler golang-go git curl \
  libevent-dev libbsd-dev libjpeg-dev pkg-config v4l-utils dosfstools \
  gstreamer1.0-tools gstreamer1.0-plugins-base gstreamer1.0-plugins-good \
  avahi-daemon libnss-mdns
apt-get clean 2>/dev/null || true   # free the download cache before the source builds
