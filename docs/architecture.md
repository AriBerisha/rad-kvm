...
- `auth.go` — single-user auth: PBKDF2-HMAC-SHA256 (600k iters, stdlib), HMAC-signed session cookie (stateless, survives restart via persisted secret), optional TOTP.
- `totp.go` — optional RFC 6238 2FA (stdlib): per-device `crypto/rand` secret, HMAC-SHA1 code verify with ±1 skew + replay guard (persisted last step), otpauth URI for the QR.
...
- `web/index.html` — embedded SPA: login/setup gate (with optional 2FA QR), then video + input capture. `web/qrcode.js` — vendored MIT QR encoder, lazy-loaded only for 2FA setup.
...
- **Auth:** first visit with no account → setup; then login. Gates `/stream`, `/snapshot`, `/ws/input`, and all `/api/devices*`+`/api/macros*`. Static page public. `/api/{status,setup,login,logout}`. Optional **TOTP 2FA** at setup: `GET /api/2fa/new` mints a per-device secret + otpauth URI (QR rendered offline); setup confirms a code before enabling; login then requires the code (replay-guarded). `status.totp` tells the login form to ask for it.
