...
- **Deploy model:** qkvm is cross-compiled off-board, so `qkvm` is
  cross-compiled (`CGO_ENABLED=0 GOOS=linux GOARCH=arm64`, static, ~6 MB) on the
  Mac and `scp`'d to `/tmp/qkvm.new`; `tmp/board-qkvm-install.sh` stops the
